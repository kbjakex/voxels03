...
pub struct Mouse {
    pressed: Box<[u32]>,
    just_released: Box<[(u32, u32)]>,
    frame_counter: u32,

    moved: bool,

    pos: Vec2,
    prev_pos: Vec2, // pos last *frame*, not pos on previous gametick! Much more useful
    delta: Vec2,
...
impl Mouse {
    pub fn new(initial_pos: Vec2) -> Self {
        Self {
            // ain't nobody got more than 32 buttons in a mouse
            pressed: vec![0; 32].into_boxed_slice(),
            just_released: vec![(0, 0); 32].into_boxed_slice(),
            frame_counter: 0,
            moved: false,
            pos: initial_pos,
